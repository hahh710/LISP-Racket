...
;; Please note, answers in 2 places below surrounded by semi-colons!
...
(define (eval-1 exp)
  (cond ((constant? exp) exp)
        ...
        ;; Checks if expression is a map-expression
        ;; (map-1 (lambda (x) (* x x)) '(1 2 3 4))
        ((map-exp? exp)
         (map (eval (cadr exp)) ; use underlying Racket eval so procedure returned in case of lambda
              (eval-1 (caddr exp)))) ; use underlying Racket's MAP

        ;; Checks if expression is an AND expression
        ((and-exp? exp)
         (cond
           ((null? (cdr exp)) ; Check if null
            #t) ; return true
           ((equal? #f (eval-1 (cadr exp))) ; check if false
            #f) ; return false
           (else (cond ((null? (cddr exp)) ; check if next expression null 
                        (eval-1 (cadr exp))) ; just evaluate current expression
                       (else (eval-1 (append '(and) (cddr exp)))))))) ; evaluate AND for rest of expressions
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; ASSIGNMENT EXERCISE 1 AND 2 HELPER FUNCTIONS
